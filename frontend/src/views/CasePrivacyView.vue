...
// 当前医生信息
const currentDoctor = ref<DoctorInfo>({
  id: 'doc001',
  name: '张医生',
  title: '主治医师',
  hospital: '北京儿童医院',
  department: '康复科',
  licenseNumber: 'BJ20240001',
  phone: '138****1234',
...
const caseRecords = ref<CaseRecord[]>([
  {
    ...
    caseType: 'online',
    title: '儿童感统失调康复训练记录',
    description: '6岁患儿前庭觉训练过程记录，展示平衡能力改善情况',
    bodyParts: ['functional', 'partial'],
    symptoms: ['平衡失调', '注意力不集中', '精细动作困难'],
    files: [
      {
        id: 'file_001',
        name: '平衡训练_局部录像.mp4',
        type: 'video',
        size: 45 * 1024 * 1024,
        uploadDate: '2024-07-08 10:30:00',
        bodyPart: 'functional',
        description: '患儿进行平衡木训练的局部动作记录',
        isEncrypted: true,
        privacyLevel: 'medium',
        thumbnail: '/api/placeholder/200/150'
      }
    ],
    doctorInfo: currentDoctor.value,
    privacySettings: {
      anonymizePatient: true,
      hidePersonalInfo: true,
      limitedAccess: true,
  ...
  },
  {
    ...
    caseType: 'offline',
    title: '手部精细动作评估',
    description: '4岁患儿手部精细动作能力评估，局部手部动作记录',
    bodyParts: ['limbs', 'partial'],
    symptoms: ['精细动作发育迟缓', '手指协调性差'],
    files: [
      {
        id: 'file_002',
        name: '手部动作_评估图片.jpg',
        type: 'image',
        size: 2 * 1024 * 1024,
        uploadDate: '2024-07-07 14:20:00',
        bodyPart: 'limbs',
        description: '手部抓握和精细动作能力展示',
        isEncrypted: true,
        privacyLevel: 'medium',
        thumbnail: '/api/placeholder/200/150'
      }
    ...
    status: 'uploaded',
    createdAt: '2024-07-07 14:00:00',
    updatedAt: '2024-07-07 16:45:00',
    tags: ['精细动作', '手部评估', '发育评估'],
    isAnonymized: true
  }
])
...
const filteredCaseRecords = computed(() => {
  ...
  if (filterType.value !== 'all') {
    records = records.filter(record => record.caseType === filterType.value)
  }
...
const statistics = computed(() => {
  ...
  const online = caseRecords.value.filter(r => r.caseType === 'online').length
  const offline = caseRecords.value.filter(r => r.caseType === 'offline').length
...
// 提交病例上传
const submitCaseUpload = () => {
  if (!uploadForm.value.title || !uploadForm.value.description || uploadForm.value.files.length === 0) {
    alert('请填写完整信息并上传文件')
    return
  }
  ...
  const newCase: CaseRecord = {
    ...
    caseType: uploadForm.value.caseType,
    title: uploadForm.value.title,
    description: uploadForm.value.description,
    bodyParts: uploadForm.value.bodyParts,
    symptoms: uploadForm.value.symptoms,
    files: uploadForm.value.files.map((file, index) => ({
      id: `file_${Date.now()}_${index}`,
      name: file.name,
      type: file.type.startsWith('image/') ? 'image' : file.type.startsWith('video/') ? 'video' : 'document',
      size: file.size,
      uploadDate: new Date().toISOString(),
      bodyPart: uploadForm.value.bodyParts[0] || 'partial',
      description: `${uploadForm.value.title} - 附件${index + 1}`,
      isEncrypted: uploadForm.value.privacySettings.encryptFiles,
      privacyLevel: selectedRule.value?.requiredPrivacyLevel || 'medium',
      thumbnail: '/api/placeholder/200/150'
    })),
    doctorInfo: currentDoctor.value,
    privacySettings: uploadForm.value.privacySettings,
    status: 'uploaded',
    createdAt: new Date().toISOString(),
    updatedAt: new Date().toISOString(),
    tags: uploadForm.value.tags,
    isAnonymized: uploadForm.value.privacySettings.anonymizePatient
  }
  
  caseRecords.value.unshift(newCase)
      ...
      <div class="verification-content">
        <div class="doctor-info">
          ...
          <div class="doctor-details">
            ...
            <p>执业证号：{{ currentDoctor.licenseNumber }}</p>
          </div>
        </div>
        ...
        >
          <div class="case-header">
            <div class="case-title-section">
              ...
              <div class="case-meta">
                <span class="case-type">{{ caseRecord.caseType === 'online' ? '线上' : '线下' }}</span>
                <span 
                  class="case-status"
                  :style="{ 
          ...
          <div class="case-content">
            <p class="case-description">{{ caseRecord.description }}</p>
            ...
            <div class="case-details">
              <div class="detail-section">
                ...
                <div class="body-parts-tags">
                  <span 
                    v-for="part in caseRecord.bodyParts" 
                    :key="part"
                    class="body-part-tag"
                  >
                    {{ bodyPartOptions.find(opt => opt.value === part)?.label || part }}
                  </span>
                </div>
              </div>
              ...
              <div class="detail-section">
                ...
                <div class="symptoms-tags">
                  <span 
                    v-for="symptom in caseRecord.symptoms.slice(0, 3)" 
                    :key="symptom"
                    class="symptom-tag"
                  >
                    {{ symptom }}
                  </span>
                  <span v-if="caseRecord.symptoms.length > 3" class="more-symptoms">
                    +{{ caseRecord.symptoms.length - 3 }}
                  </span>
                </div>
              </div>
            </div>
            ...
            <div class="case-files">
              <h5>附件文件 ({{ caseRecord.files.length }})：</h5>
                ...
                >
                  ...
                  <div class="file-info">
                    ...
                    <div class="file-meta">
                      ...
                      <span class="file-privacy">
                        🔒 {{ getPrivacyLevelInfo(file.privacyLevel).text }}
                      </span>
                    </div>
                  </div>
                </div>
            ...
            <div class="privacy-indicators">
              <div class="privacy-item" v-if="caseRecord.isAnonymized">
                <span class="privacy-icon">🔒</span>
                <span class="privacy-text">已匿名化</span>
              </div>
              <div class="privacy-item" v-if="caseRecord.privacySettings.encryptFiles">
                <span class="privacy-icon">🛡️</span>
                <span class="privacy-text">文件已加密</span>
              </div>
              <div class="privacy-item" v-if="caseRecord.privacySettings.watermark">
                <span class="privacy-icon">💧</span>
                <span class="privacy-text">已加水印</span>
              </div>
            </div>
          </div>
          ...
          <div class="case-actions">
            ...
            <button class="action-btn edit-btn" v-if="caseRecord.status === 'draft'">编辑</button>
      ...
      <div class="upload-modal" @click.stop>
        ...
        <div class="modal-content">
          <form @submit.prevent="submitCaseUpload" class="upload-form">
            ...
            <div class="form-section">
              <h3>涉及部位</h3>
              <div class="body-parts-selection">
                <label 
                  v-for="option in bodyPartOptions.filter(opt => selectedRule?.allowedBodyParts.includes(opt.value))" 
                  :key="option.value"
            ...
            <div class="form-section">
              ...
              <div class="privacy-settings">
                <label class="checkbox-label">
                  <input type="checkbox" v-model="uploadForm.privacySettings.anonymizePatient">
                  <span>患者信息匿名化</span>
                </label>
                <label class="checkbox-label">
                  <input type="checkbox" v-model="uploadForm.privacySettings.encryptFiles">
                  <span>文件加密存储</span>
                </label>
                <label class="checkbox-label">
                  <input type="checkbox" v-model="uploadForm.privacySettings.watermark">
                  <span>添加医院水印</span>
                </label>
                <label class="checkbox-label">
                  <input type="checkbox" v-model="uploadForm.privacySettings.limitedAccess">
                  <span>限制访问权限</span>
                </label>
              </div>
            </div>
