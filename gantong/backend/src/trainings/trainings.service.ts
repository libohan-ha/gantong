...
type DoctorUpcomingTrainingRow = {
...
export class TrainingsService {
  constructor(
    ...
    @InjectRepository(TrainingBooking)
    private bookingRepo: Repository<TrainingBooking>,
  ) {}
  ...
  async findOneForDoctor(id: number, doctorUserId: number) {
    ...
    if (training.doctorUserId !== doctorUserId)
  ...
  async listDoctorsWithUpcomingTrainings(page = 1, pageSize = 10, q?: string) {
    ...
    const rows = rawRows as DoctorUpcomingTrainingRow[];
    return {
      items: rows.map((r) => ({
