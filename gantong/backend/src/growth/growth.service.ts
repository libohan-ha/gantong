...
export class GrowthService {
  constructor(
    ...
    @InjectRepository(GrowthProfile)
    private profileRepo: Repository<GrowthProfile>,
  ...
  private async ensureChildOwnership(
    ...
    if (child.parentUserId !== userId)
      throw new ForbiddenException('无权访问该孩子');
    return child;
  }

  // children
  async listChildren(userId: number) {
    const items = await this.childRepo.find({
