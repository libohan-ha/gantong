...
export async function uploadCase(params: {
  title: string
  description?: string
  caseType?: CaseType
  files: File[]
}): Promise<UploadCaseResponse> {
  ...
  if (params.caseType) {
    form.append('caseType', params.caseType)
  }
...
export async function getMyCases(query?: {
  page?: number
  pageSize?: number
}): Promise<{ items: BackendCaseRecord[]; total: number; page: number; pageSize: number }> {
...
export async function updateCase(
  id: number,
  data: { title?: string; description?: string | null; caseType?: CaseType },
): Promise<BackendCaseRecord> {
  const res = await api.patch(`/cases/${id}`, data)
  return res.data
}

export async function addCaseFiles(
  id: number,
  files: File[],
): Promise<BackendCaseRecord> {
  const form = new FormData()
  files.forEach((f) => form.append('files', f))
  const res = await api.post(`/cases/${id}/files`, form, {
    headers: { 'Content-Type': 'multipart/form-data' },
  })
  return res.data
}

export async function removeCaseFile(
  caseId: number,
  fileId: number,
): Promise<BackendCaseRecord> {
  const res = await api.delete(`/cases/${caseId}/files/${fileId}`)
  return res.data
}
